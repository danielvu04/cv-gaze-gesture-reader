...
from modules.metrics import MetricsTracker, GestureMetricsTracker
...
EXIT_HOLD_FRAMES = 60
FRAME_SKIP = 1          # process every frame; set to 1,2,3 to trade FPS vs CPU
SCROLL_COOLDOWN = 10    # frames to wait after a scroll before allowing another
...
class ReadingPipeline(QThread):
    ...
    gestureRecognized = pyqtSignal(str)    # e.g., "thumbs_up", "swipe_down"
    ...
    def __init__(self, monitor_index=1, parent=None):
        ...
        self.gesture_metrics = GestureMetricsTracker()
    ...
    def run(self):
        ...
        # Reduce resolution to cut CPU cost
        cam_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cam_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 360)
        ...
        try:
            ...
            while self.running:
                ...
                # Skip heavy processing some frames
                do_process = (self.frame_idx % FRAME_SKIP == 0)

                # Gaze
                if do_process:
                    gaze_cam = gaze_tracker.process(cam_frame)
                    last_gaze_cam = gaze_cam
                else:
                    gaze_cam = last_gaze_cam
                ...
                if do_process:
                    ...
                    last_gesture_info = gesture_info
                else:
                    gesture_info = last_gesture_info or {
                        "pinch": False,
                        "is_pinch": False,
                        "swipe_up_trigger": False,
                        "swipe_down_trigger": False,
                        "open_palm": False,
                        "thumbs_up_trigger": False,
                        "thumbs_down_trigger": False,
                        "landmarks": None,
                    }
                ...
                if gesture_info["is_pinch"] and self.scroll_cooldown == 0:
                    ...
                    elif gesture_info["swipe_down_trigger"]:
                ...
                # Pinch alignment metric (use active region)
                if gesture_info["pinch"]:
                    self.metrics.log_pinch(
                        pinch_region_index=fusion.active_index,
                        active_region_index=fusion.active_index,
                    )
